...
class BillboardBatch {
    ...
    use_shader(shader: Shader) {
        if (!this.in_use) new Error("BillboardBatch::use_shader");
    ...
    begin() {
        if (this.in_use) throw new Error("BillboardBatch::begin");
        this.in_use = true;
        ...
        if (!this.in_use) throw new Error("BillboardBatch::end");
        this.in_use = false;
    ...
    */
    draw_spritesheet(texture: Texture, position: vec3, size: vec3, _coords: vec4, direction: boolean, color: vec4): void {
        this.draw_spritesheet_rotation(texture, position, size, _coords, direction, 0, color);
    }

    draw_spritesheet_rotation(texture: Texture, position: vec3, size: vec3, _coords: vec4, direction: boolean, rotation: number, color: vec4): void {
        // TODO: check if we have added the texture before
        this.textures.push({ index: this.textures.length, texture: texture });
    ...
    draw_chat_bubble(chat: ChatBubble, position: vec3, _size: vec2, direction: boolean): void {
    ...
    draw_image(texture: Texture, position: vec3, size: vec2, direction: boolean): void {
        // TODO: check if we have added the texture before
        this.textures.push({ index: this.textures.length, texture: texture });
