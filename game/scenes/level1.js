...
var Level1 = (function (_super) {
    ...
    function Level1(engine) {
        _super.call(this, engine, "level1");
        this.world = new World(this.engine);
        this.create_world();
    }
    Level1.prototype.create_world = function () {
        var x = 0;
        var y = 0;
        var z = 0;
        var map = [];
    ...
    Level1.prototype.add_chat_command = function (chat, follow, complete) {
    ...
    Level1.prototype.remove_chat_command = function (enter, space, x) {
    ...
    Level1.prototype.button_x = function () {
        this.remove_chat_command(false, false, true);
    };
    Level1.prototype.button_space = function () {
    ...
    Level1.prototype.render = function (time, dt) {
        render.call(this, time, dt, this.world);
    };
    return Level1;
})(Scene);
function render_background(time, world) {
    ...
    for (var b in current_dimension.background_textures) {
        ...
        billboard_batch.draw_spritesheet(tex.animation.spritesheet, position, (new vec3([tex.animation.width_per_frame, tex.animation.height_per_frame, 1])).mul((1.0 / 16.0) * tex.scale), tex.animation.coords, true, new vec4([1, 1, 1, 1 - world_rotation_change]));
    }
    billboard_batch.end();
}
function render_objects(time, deformation, world, fixed_view_matrix) {
    var current_dimension = world.dimensions[this.world.player.dimension];
    var world_rotation_change = (world.current_view_angle / 90.0);
    billboard_batch.use_shader(billboard_3d_shader);
    billboard_3d_shader.setMatrix4("projection", projection_matrix);
    ...
    billboard_3d_shader.setInteger("sampler0", 0);
    billboard_batch.begin();
    var temp = world.gameobjects.slice();
    world.push_particles(temp);
    // NOTE: Sort by z
    var keys = Object.keys(temp);
    var values = keys.map(function (key) { return temp[key]; });
        ...
        if (obj instanceof Player) {
            ...
            if (pobj.animation_state == PlayerAnimationState.Attack) {
                ...
                if (ani.current_frame == 2) {
                    ...
                    billboard_batch.draw_spritesheet_rotation(weapon.animation.spritesheet, weapon_position, weapon.scale, weapon.animation.coords, !pobj.direction, weapon.rotation, pobj.color);
                }
            }
            var animator = pobj.current_animator();
            billboard_batch.draw_spritesheet_rotation(animator.spritesheet, draw_position, new vec3([pobj.scale, pobj.scale, pobj.scale]), animator.coords, pobj.direction, pobj.rotation, pobj.color);
        }
            ...
            if (k1obj.animation_state == Knight1AnimationState.Attack) {
                ...
                if (ani.current_frame == 2) {
                    ...
                    weapon_position.add((new vec3([0, weapon.offset.y, 0])).mul(1 / 16));
                    billboard_batch.draw_spritesheet_rotation(weapon.animation.spritesheet, weapon_position, weapon.scale, weapon.animation.coords, !k1obj.direction, weapon.rotation, k1obj.color);
                }
            }
            billboard_batch.draw_spritesheet_rotation(k1obj.animation.spritesheet, draw_position, new vec3([k1obj.scale, k1obj.scale, k1obj.scale]), k1obj.animation.coords, k1obj.direction, k1obj.rotation, k1obj.color);
        }
        else if (obj instanceof Boss) {
            var bobj = obj;
            if (bobj.stage == BossStage.Sword) {
                billboard_batch.draw_spritesheet_rotation(bobj.animation.spritesheet, draw_position, new vec3([bobj.scale, bobj.scale, bobj.scale]), bobj.animation.coords, bobj.direction, bobj.rotation, bobj.color);
                if (bobj.animation_state == BossAnimationState.Sword) {
                    ...
                    if (ani.current_frame == 2) {
                        ...
                        weapon_position.add((new vec3([0, weapon.offset.y, 0])).mul(1 / 16));
                        billboard_batch.draw_spritesheet_rotation(weapon.animation.spritesheet, weapon_position, weapon.scale, weapon.animation.coords, !bobj.direction, weapon.rotation, bobj.color);
                    }
                }
            }
            else {
                billboard_batch.draw_spritesheet_rotation(bobj.animation.spritesheet, draw_position.copy().add(new vec3([0, 1.0, 0])), new vec3([bobj.scale * 3, bobj.scale * 2, bobj.scale]), bobj.animation.coords, bobj.direction, bobj.rotation, bobj.color);
            }
        }
        else if (obj instanceof AnimatedObject) {
            var aniobj = obj;
            billboard_batch.draw_spritesheet_rotation(aniobj.texture, draw_position, new vec3([aniobj.scale, aniobj.scale, aniobj.scale]), aniobj.animation.coords, aniobj.direction, aniobj.rotation, aniobj.color);
        }
        else if (obj instanceof ItemObject) {
            var itemobj = obj;
            billboard_batch.draw_spritesheet_rotation(itemobj.texture, draw_position, new vec3([itemobj.scale, itemobj.scale, itemobj.scale]), itemobj.animation.coords, itemobj.direction, itemobj.rotation, itemobj.color);
        }
        else if (obj instanceof ParticleObject) {
            var ppobj = obj;
            billboard_batch.draw_spritesheet_rotation(ppobj.texture, draw_position, ppobj.size.copy().mul(ppobj.scale), new vec4([0, 0, 1, 1]), ppobj.direction, ppobj.rotation, ppobj.color);
        }
        else {
            billboard_batch.draw_spritesheet_rotation(obj.texture, draw_position, obj.size.copy().mul(obj.scale), new vec4([0, 0, 1, 1]), obj.direction, obj.rotation, obj.color);
        }
    }
    ...
    if (world.player.found_item != null) {
        billboard_batch.draw_spritesheet_rotation(world.player.found_item.animation.spritesheet, world.player.found_item_position, new vec3([1, 1, 1]), world.player.found_item.animation.coords, true, world.player.found_item_rotation, new vec4([1, 1, 1, 1]));
    }
    billboard_batch.end();
    if (world.player.inventory.length > 0 || world.player.time_shifter != null) {
        ...
        for (var item_i in world.player.inventory) {
            ...
            billboard_batch.draw_spritesheet(item.animation.spritesheet, item_position, new vec3([0.5, 0.5, 0.5]), item.animation.coords, true, new vec4([1, 1, 1, 1]));
        }
        ...
        if (world.player.time_shifter != null) {
            ...
            item_position.mul(world.view_normal);
            item_position.y += inv_size.y * 0.5 + 4 / 100;
            billboard_batch.draw_spritesheet(world.player.time_shifter.animation.spritesheet, item_position, new vec3([0.5, 0.5, 0.5]), world.player.time_shifter.animation.coords, true, new vec4([1, 1, 1, 1]));
        }
        billboard_batch.end();
    }
}
function render_world(time, deformation, world) {
    var current_dimension = world.dimensions[this.world.player.dimension];
