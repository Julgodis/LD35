...
var LoseScene = (function (_super) {
    ...
    function LoseScene(engine) {
        _super.call(this, engine, "lose");
    ...
    LoseScene.prototype.update = function (time, dt) {
    ...
    LoseScene.prototype.render = function (time, dt) {
        var gl = engine.webgl;
        ...
        billboard_batch.draw_spritesheet_rotation(player_texture, new vec3([0, 0.25, 0]), new vec3([1, 1, 1]), new vec4([0, 0, 1.0 / 8.0, 1.0 / 8.0]), true, 1 / 4 + Math.sin(engine.time / 200.0) * 0.01, this.color);
        for (var pi in this.blood.particles) {
            var particle = this.blood.particles[pi];
            billboard_batch.draw_spritesheet_rotation(particle.texture, particle.position.copy().add(new vec3([-0.1, 0.2, 0])), this.blood.size.copy().mul(particle.scale), new vec4([0, 0, 1, 1]), true, particle.rotation, particle.color.copy().mul(this.color));
        }
        billboard_batch.end();
...
//# sourceMappingURL=lose_scene.js.map