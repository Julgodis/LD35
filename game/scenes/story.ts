interface StoryScene {
    add_chat_command(chat: ChatBubble, follow: GameObject, complete: (command: ChatCommand) => void);
    chat_destory();
...
class Story extends Scene implements StoryScene {
    world: World
    ...
    create_world() {
        ...
        mapCtx.drawImage((<ImageTexture>map_texture).image, 0, 0, map_texture.size.x, map_texture.size.y, 0, 0, map_texture.size.x, map_texture.size.y);
            ...
            for (var y = 0; y < map_texture.size.y / 2; y++) {
                for (var x = 0; x < map_texture.size.x; x++) {
                    ...
                    if (a > 128) {
                        ...
                        if (r == 128 && g == 128 && b == 128) tile_id = TileData.STONE_01;
                        else if (r == 64 && g == 64 && b == 64) tile_id = TileData.WALL_01;
                        else if (r == 0 && g == 255 && b == 0) tile_id = TileData.DIM01_GROUND_GRASS;
                        else if (r == 0 && g == 128 && b == 0) tile_id = TileData.DIM01_GROUND;
                        else if (r == 255 && g == 128 && b == 0) tile_id = TileData.DIM02_GROUND_GRASS;
                        else if (r == 128 && g == 64 && b == 0) tile_id = TileData.DIM02_GROUND;
                        else if (r == 128 && g == 128 && b == 0) tile_id = TileData.SLOPE_0r1;
                        else if (r == 128 && g == 128 && b == 1) tile_id = TileData.SLOPE_0r3;
                        else if (r == 64 && g == 128 && b == 0) tile_id = TileData.SLOPE_1r1;
                        else if (r == 64 && g == 128 && b == 1) tile_id = TileData.SLOPE_1r3;
                        else if (r == 0 && g == 10 && b == 255) tile_id = TileData.WATER_01;
                        else if (r == 0 && g == 0 && b == 255) tile_id = TileData.WATER_02;
                        else if (r == 10 && g == 10 && b == 255) tile_id = TileData.WATER_11;
                        else if (r == 10 && g == 0 && b == 255) tile_id = TileData.WATER_12;
                            ...
                            var son_idle_animation = this.son.animation.add_animation(
                                "idle",
                            ...
                            var son_dead_animation = this.son.animation.add_animation(
                                "dead",
                                Animation.fsequence(16, 4),
                                Animation.ft_repate(4, Animation.FrameTime * 5.0),
                                false);
        ...
        for (var i = 0; i < 150; i++) {
            ...
            var animation = bg_tex.animation.add_animation(
                "idle",
                Animation.fsequence(0, 4),
                Animation.ft_repate(4, 500.0),
                true);
...

        {
            for (var i = 0; i < 25; i++) {
                ...
                var animation = bg_tex.animation.add_animation(
                    "idle",
                    Animation.fsequence(0, 1),
                    Animation.ft_repate(4, 300.0),
                    true);
        ...
        (new Tween(this.son)).to({}, 1000.0).delay(500.0).ease(Easing.quadratic_inout).start(engine.time).on(function (value: number) {
    ...
    start_story_text() {
        ...
        this.add_chat_command(chat, this.son, function (command: ChatCommand) {
            var chat1 = new ChatBubble(engine, new vec2([150, 0]));
            chat1.text = "WTF!?";

            this.add_chat_command(chat1, this.world.player, function (command: ChatCommand) {
                var chat1 = new ChatBubble(engine, new vec2([300, 0]));
                chat1.text = "My name is Theodor and I am from the FUTURE!";

                this.add_chat_command(chat1, this.son, function (command: ChatCommand) {
                    var chat1 = new ChatBubble(engine, new vec2([300, 0]));
                        ...
                        this.add_chat_command(chat1, this.son, function (command: ChatCommand) {
                            ...
                            this.add_chat_command(chat1, this.world.player, function (command: ChatCommand) {
                                ...
                                this.add_chat_command(chat1, this.son, function (command: ChatCommand) {
                                    ...
                                    this.add_chat_command(chat1, this.son, function (command: ChatCommand) { this.kill_son() }.bind(this));
                                }.bind(this));
                            }.bind(this));
                        }.bind(this));
    ...
    update(time: number, dt: number) {
        if (this.state == StoryState.Intro && this.world.player.dimension != 0) {
            ...
            (new Tween(this)).to({}, 3000.0).on(function (value: number) {
